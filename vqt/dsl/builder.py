...
class CircuitBuilder:
    ...
    def h(self, qubit_idx: int):
        gate = Gate("h", 1)
        self.circuit.append(gate, [Qubit(self.qreg, qubit_idx)])
    ...
    def cx(self, control_idx: int, target_idx: int):
        gate = Gate("cx", 2)
        self.circuit.append(gate, [Qubit(self.qreg, control_idx), Qubit(self.qreg, target_idx)])
    
    def swap(self, qubit1_idx: int, qubit2_idx: int):
        gate = Gate("swap", 2)
        self.circuit.append(gate, [Qubit(self.qreg, qubit1_idx), Qubit(self.qreg, qubit2_idx)])

    def measure(self, qubit_idx: int, clbit_idx: int):
        gate = Gate("measure", 1)
        self.circuit.append(gate, [Qubit(self.qreg, qubit_idx)], [Clbit(self.creg, clbit_idx)])
